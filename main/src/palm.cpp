...
int main(int argc, char **argv){
    ros::init(argc, argv, "palm");
    tf::TransformListener listener;
    ros::NodeHandle nh;
    listener.waitForTransform("/panda_link0", "/palm_link", ros::Time(), ros::Duration(1.0));
    ros::Rate loop_rate(100);
    ros::Publisher palm = nh.advertise<geometry_msgs::PoseStamped>("/link0_to_palm", 100);
    ...
    while (ros::ok()){
      try
      {
        ...
        listener.lookupTransform("/panda_link0", "/palm_link", ros::Time(), echo_transform);
